...
def total():
    if Bread.get()==0 and Wine.get()==0 and Rice.get()==0 and Gal.get()==0:
        ...
        t=float(b*20+w*6+r*38+g*25)
        Total.set(b + w + r + g)
        total_cost.set('₹ ' + str(round(t, 2)))

        cb.set('₹ '+str(round(b * 20, 2)))
        cw.set('₹ '+str(round(w*40,2)))
        cr.set('₹ '+str(round(r*38,2)))
        cg.set('₹ '+str(round(g*25,2)))
...
def receipt():
    ...
    textarea.insert(END,'Name of Customer: \n\n')
    textarea.insert(END,'Contact Number: \n\n')
...
title=Label(root,pady=5,text="Billing System",bd=12,bg=bg_color,fg='white',font=('times new roman', 35 ,'bold'),relief=GROOVE,justify=CENTER)
title.pack(fill=X)
...
wine=Label(F1, text='Egg', font=('times new rommon',20, 'bold'), fg='lawngreen',bg=bg_color)
